...
def get_ward_distribution(case_dict, file_path, write_mode = 'w', csv_sep = ';'):
    ...
    This information is extracted from the Case().moves attribute, since each Move() object has an attribute Move().ward . The written file contains
    --> Ward names
    --> Occurrence counts
    ...
    for each_case in case_dict.values():
        for each_move in each_case.moves.values():
            if each_move.ward is None:
                continue
            if each_move.ward.name not in ward_count.keys():
                ward_count[each_move.ward.name] = 1
            else:
                ward_count[each_move.ward.name] += 1
...
def collect_patient_info(pat_dict, file_path, write_mode = 'w', csv_sep = ';'):
    ...
    for pat_tuple in pat_dict.items():
        ...
        for each_case in pat_tuple[1].cases:
            ...
            move_count += len(list(each_case.moves.keys()))
            surgery_count += len(each_case.surgeries)
            medication_count += len(each_case.medications)
        ### Add all extracted measures to pat_counts
        pat_counts[pat_tuple[0]]['Apmnt Count'] = apmnt_count
        pat_counts[pat_tuple[0]]['Apmnt Duration'] = apmnt_duration
        pat_counts[pat_tuple[0]]['Move Count'] = move_count
    ...
    with open(file_path, write_mode) as write_file:
        write_file.write(f"Patient_ID{csv_sep}Case_Count{csv_sep}Appointment_Count{csv_sep}Appointment_Duration{csv_sep}Move_Count{csv_sep}Surgery_Count{csv_sep}Medication_Count\n")
        for each_tuple in pat_counts.items():
            data_list = [each_tuple[0]]
            data_list.append(each_tuple[1]['Case Count'])
            data_list.append(each_tuple[1]['Apmnt Count'])
            data_list.append(each_tuple[1]['Apmnt Duration'])
            data_list.append(each_tuple[1]['Move Count'])
            data_list.append(each_tuple[1]['Surgery Count'])
            data_list.append(each_tuple[1]['Medication Count'])
