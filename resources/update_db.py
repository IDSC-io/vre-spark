...
for each_file in sql_files:
    ...
    WriteSQLToCSV(path_to_sql=os.path.join(SQL_DIR, each_file), path_to_csv=os.path.join(CSV_DIR, each_file.replace('.sql', '.csv')), csv_sep=CSV_DELIM,
                  connection_file="T:/IDSC Projects/VRE Model/Connection File/Server_Connection.txt", trusted_connection=True)

    # subprocess.call(['java', '-jar', JAR_FILEPATH, os.path.join(SQL_DIR, each_file), os.path.join(CSV_DIR, each_file.replace('.sql', '.csv'))])
