...
class Neo4JExporter:
    ...
    def write_patient_patient(self, contact_pats, patients):
        ...
        with open(os.path.join(self.config['PATHS']['neo4j_dir'], 'patient_patient_' + self.init_date + '.csv'), "w") as csvfile:
            ...
            for contact in contact_pats:  # list of tuples --> can be directly printed !
                if patients[contact[0]].get_relevant_case() is not None and patients[
                    contact[1]].get_relevant_case() is not None:  # make sure to include only patients with a relevant date !
    ...
    def write_device(self, devices):
        ...
        :param devices: Dictionary mapping device names to a Device() object --> {'BH N 125' : Device(), ... }
        """
        device_count = 0
